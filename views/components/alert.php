...
$class   = 'alert alert-' . $variant . ($extra_class ? ' ' . $extra_class : '');
$icon_path = $icons[$variant] ?? $icons['info'];
?>
<div class="<?php echo $class; ?>"<?php echo $id_attr; ?> role="alert">
  <span class="alert-icon">
    <svg viewBox="0 0 20 20" fill="currentColor">
      <?php echo $icon_path; ?>
    </svg>
  </span>
  <span style="flex:1;"><?php echo $message; ?></span>
  <?php if ($dismissible): ?>
